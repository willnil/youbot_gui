...
pts = deque(maxlen=args["buffer"]) # pointer
counter = 0
(dX, dY) = (0, 0)
direction = ""
...
while True:
	...
	if len(cnts) > 0:
		...
		# it to compute the minimum enclosing rect and centroid
		...
		# draw the rect/circle and centroid on the frame,
		...
		pts.appendleft(center) # global variable
	...
	for i in np.arange(1, len(pts)):
		...
		if counter >= 10 and i == 1 and pts[-10] is not None:
			...
			# x-direction
			if np.abs(dX) > 20: # more than 20 pixel btw x-Coordiantes
